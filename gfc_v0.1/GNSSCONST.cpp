...
namespace gfc
{
    ...
    //调用常量的初始化函数；必须在类外调用
    std::map< GString,  long double > gfc::GNSSCONST::constantValue = gfc::GNSSCONST::Initializer();
    ...
    /*
     ...
     速度单位为：m/s
     */
    std::map< GString,  long double > GNSSCONST::Initializer()
    {
        ...
        tmpconsValue["AU"]       = 149597870660.0;  //天文单位
        ...
        tmpconsValue["SECPRAD"]  = 180.0*3600.0/GetByName_internal("PI",tmpconsValue);  //每一弧度有206264.8秒
    ...
    /*静态函数：取出变量的值*/
    long double GNSSCONST::GetByName(GString variableName)
    {
    ...
    /*静态函数：取出变量的值*/
    long double GNSSCONST::GetByName_internal(GString variableName,std::map< GString,  long double >& myconstantValue)
    {
    ...
    void GNSSCONST::RegByName(GString variableName,long double variableValue)
    {
        if( constantValue.count(variableName) == 0 )
        {
    ...
    /*删除一个常量*/
    void GNSSCONST::UnregByName(GString variableName)
    {
        if( constantValue.count(variableName)>0 )
        {
    ...
    void GNSSCONST::dump( std::ostream& s ) 
    {
        s<<"ClassName:    "<<"GNSSCONST"<<std::endl;
